...
from protein_holography_pytorch.utils import cartesian_to_spherical__numpy
...
def get_neighborhoods_from_protein(
    ...
    if not (res_ids_selection is None):
        equals = np.all(res_ids_selection.reshape(-1, 6, 1) ==
                        nh_ids.transpose().reshape(1, 6, -1), axis=1)
    ...
    #         print(nh_id)
            ...
            # neighbors_list[i] = [
            #     x for x in neighbor_list if 
            #     np.logical_or.reduce(res_ids[x] != nh_id, axis=-1)]
            # print(f"Shape of neighbor list: {neighbor_list.shape}")
            # print(f"Shape of res_ids: {res_ids.shape}")
            central_locs = np.logical_and.reduce(
                res_ids[neighbor_list] == nh_id[None,:],axis=-1)
            neighbors_list[i] = neighbor_list[~central_locs]

    # keep central residue but still remove central CA
    else:
        for i, (nh_id, neighbor_list) in enumerate(zip(nh_ids, neighbors_list)):
            central_locs = np.logical_and.reduce(res_ids[neighbor_list] == nh_id[None,:], axis=-1)
            CA_locs = atom_names[neighbor_list] == b' CA '
            neighbors_list[i] = neighbor_list[~np.logical_and.reduce(
                np.stack([central_locs, CA_locs]), axis=0)]
